...
void Darcy<dim>::output_velocity_at_observation_points_npy(
  ...
  const auto data_array = VectorTools::point_values<dim>(
      dummy_mapping, dof_handler, solution_distributed, spatial_coordinates,
      evaluation_cache);
...
void Darcy<dim>::output_field_at_observation_points_npy(
  ...
  const auto data_array =
      VectorTools::point_values<1>(remote_eval_obj, rf_dof_handler, x_vec_distributed);
...
void Darcy<dim>::output_full_velocity_npy(const std::string &output_path) {
  ...
  for (unsigned int i = 0; i < dof_handler.n_dofs(); ++i) {
    double temp;
    temp = Utilities::MPI::reduce<double>(
        solution.in_local_range(i) ? solution[i] : 0.0, MPI_COMM_WORLD,
        custom_fuse, 0);
