...
1) `darcy_adjoint.cc` $\rightarrow$ `darcy_adjoint`: The associated adjoint problem that returns the derivative $\frac{\partial \mathcal{L}(f(\boldsymbol{x}))}{\partial \boldsymbol{x}}$ for an objective function $\mathcal{L}(f(\boldsymbol{x}))$

## Setup, installation, build and dependencies

This code requires the installation and setup of [`deal.II`](https://www.dealii.org/) as the main library to solve the governing equations.
The easiest way to install `deal.II` is via the [`spack`](https://spack.readthedocs.io/en/latest/index.html) package manager.

First install `spack` by cloning the repository and setting up the environment:
...
Using `spack` to install `deal.II` will build and install all its dependencies according to your compiler. Simply run
...
cd build
cmake \
    -DDEAL_II_DIR={spack dealii install directory} \
