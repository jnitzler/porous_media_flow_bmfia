...
void Darcy<dim>::read_primary_solution(const std::string &output_path) {
  ...
  npy::LoadArrayFromNumpy(filename, shape, fortran_order, tmp_primary_solution);
    ...
    if (cell->is_locally_owned()) {
      ...
      for (unsigned int k = 0; k < data_element_idx.size(); ++k) {
        ...
        Point<dim> current_cell_point =
            dummy_mapping.transform_real_to_unit_cell(cell,
                                                      spatial_coordinates[idx]);
        ...
        for (unsigned int j = 0; j < dofs_per_cell_rf; ++j) {
          ...
          grad_log_lik_x_partial_distributed[local_dof_indices_rf[j]] +=
  ...
  for (const auto &cell_tria : triangulation.active_cell_iterators()) {
    ...
    if (cell->is_locally_owned()) {
      ...
      for (unsigned int q = 0; q < n_q_points; ++q) {
        ...
        std::vector<Tensor<1, dim>> fe_values_velocities_vec(dofs_per_cell);
        for (unsigned int ii = 0; ii < dofs_per_cell; ++ii) {
          fe_values_velocities_vec[ii] = fe_values[velocities].value(ii, q);
        }
        ...
        for (unsigned int k = 0; k < rf_dofs_per_cell; ++k) {
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i) {
            const Tensor<1, dim> phi_i_u = fe_values_velocities_vec[i];
            ...
            for (unsigned int j = 0; j < dofs_per_cell; ++j) {
              const Tensor<1, dim> phi_j_u = fe_values_velocities_vec[j];
              ...
              grad_log_lik_x_distributed[local_rf_dof_indices[k]] +=
                  local_solution_i_JxW * (pre_mult * phi_j_u) * local_primary_j;
