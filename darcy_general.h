...
void Darcy<dim>::setup_grid_and_dofs() {
  ...
  x_vec_distributed.reinit(partitioning_rf, relevant_partitioning_rf,
                           MPI_COMM_WORLD);
}
...
void Darcy<dim>::solve() {
  ...
  SolverControl solver_control_system(
      system_matrix.m(), 1.0e-10 * system_rhs.l2_norm(), true, 1.0e-10);
